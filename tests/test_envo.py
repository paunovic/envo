...
def test_credentials_never_appear_in_the_command_argv(
    ...
    _, argv, _ = executed[0]
    assert not any(
        "AKIA" in part or "secret" in part or "=" in part and part.startswith(("AWS_", "APP_"))
        for part in argv
    )
...
def test_a_failed_materialization_names_the_variable_and_parameter(
    ...
    (repo / "pyproject.toml").write_text(
        "[tool.envo.environments.qa.vars]\n"
        'APP_DATABASE_URL = "/database/missing"\n',
    )
    monkeypatch.chdir(repo)
    ...
    assert exit_code == 1
    assert "APP_DATABASE_URL" in stderr
...
def test_declared_vars_resolve_in_one_bulk_call(
    ...
    assert env["API_TOKEN"] == "token-value"
...
def test_shared_vars_apply_to_every_environment(
    ...
    assert executed[0][2]["APP_DATABASE_URL"] == "postgresql://shared"
    assert executed[1][2]["APP_DATABASE_URL"] == "postgresql://shared"
...
def test_an_environment_overrides_a_shared_var(
    ...
    (repo / "pyproject.toml").write_text(
        ...
        'APP_DATABASE_URL = "/database/staging/url/master"\n',
    )
    monkeypatch.chdir(repo)
    monkeypatch.setattr(
        cli.subprocess,
    ...
    assert executed[0][2]["APP_DATABASE_URL"] == "postgresql://overridden"
